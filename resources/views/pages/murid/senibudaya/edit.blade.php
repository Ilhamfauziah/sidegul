<form action="{{ route('senibudaya.update', $edit->id) }}" method="POST" enctype="multipart/form-data">
    @method('PUT')
    @csrf
    <div class="mb-3 mt-2">
    ...
    <div class="mb-3 mt-2">
        <label for="senbud_id" class="form-label">Ekstrakurikuler</label>
        <select class="form-select mb-3" name="senbud_id" required>
            <option selected>{{ $edit->senbud_id }}</option>
            @foreach ($m_senbud as $key =>$value )
            <option value="{{ $value->id }}">{{ $value->name_senbud }}</option>
            @endforeach
        </select>
    </div>
    <div class="mb-3 mt-2">
